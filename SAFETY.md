...
- The Steam launch option wraps the game as `"…\Elden Ring Backuptool.exe" --monitor %command%`.
...
1. Stream the source `.sl2` (and `.sl2.bak`) into a deflate `save.zip` while hashing the original
   bytes with SHA-256. Per-file and total size limits prevent corrupted files from exhausting memory.
2. Write the archive inside a uniquely named **temporary folder under the
...
5. Only after verification: write `metadata.json` (recording hashes of the **original**,
   uncompressed bytes and of the complete ZIP), then atomically rename the temp folder into place.
...
- Existing archives are hashed before the app trusts them for display, deduplication, or retention.

## Retention

- Retention keeps the newest N integrity-validated snapshots **per account** and only ever deletes finalized snapshot
...
- The backup destination may not be inside the live save folder, and the save folder may not be
  inside the backup destination (checked in both directions after resolving existing symlinks and
  Windows junctions). The GUI and background monitor enforce the same check. This prevents backups
  from backing up backups, or a delete touching the live save.
